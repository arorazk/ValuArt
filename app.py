...
All inputs live on the main page (no sidebar) so nothing can collapse.
"""
...
def load_or_train():
    ...
    return joblib.load(MODEL_PATH), prep


def predict_price(model, prep, inputs: dict) -> float:
    row = pd.DataFrame([inputs])
    row["sale_price"] = 1.0
    X, _ = prep.transform(engineer_features(row))
...
<div style="background:linear-gradient(135deg,#1a1a2e,#16213e,#0f3460);
            padding:2.2rem 2rem 1.8rem; border-radius:14px; margin-bottom:1.8rem;">
    <h1 style="color:#e94560;margin:0;font-size:2.4rem;font-weight:800;letter-spacing:-1px;">
        🎨 ValuArt
    </h1>
    <p style="color:#a8b2d8;margin:0.3rem 0 0;font-size:1.1rem;">
...
st.markdown("### 🔧 NFT Listing Details")
st.caption("Fill in the details below and click Predict Sale Price.")
...
with c2:
    st.markdown("**Price & Market**")
    floor_price     = st.slider("Floor Price (ETH)", 0.01, 150.0, 10.0, step=0.1)
    past_avg_price  = st.slider("Past Avg Sale Price (ETH)", 0.01, 300.0, 15.0, step=0.1)
...
with c3:
    ...
    engagement_rate   = st.slider("Engagement Rate", 0.001, 0.15, 0.035, step=0.001, format="%.3f")
...
with col_pred:
    ...
    if predict_btn:
        inputs = {
            "collection_name": collection_name, "creator": creator,
            "blockchain": blockchain, "rarity_score": rarity_score,
            "trait_count": trait_count, "floor_price": floor_price,
            "past_avg_price": past_avg_price, "sales_last_7d": sales_last_7d,
            "sales_last_30d": sales_last_30d, "listing_count": listing_count,
            "days_since_mint": days_since_mint, "twitter_followers": twitter_followers,
            "discord_members": discord_members, "engagement_rate": engagement_rate,
        }
        ...
        <div style="background:#1e293b;border:1px dashed #334155;border-radius:12px;
            ...
            <p style="margin:0.5rem 0 0;">Fill in the details above and click <strong>Predict Sale Price</strong></p>
        </div>
        """, unsafe_allow_html=True)
...
with st.expander("📖  How it works", expanded=False):
    ...
    **Pipeline:** Data generation → Feature engineering → Preprocessing → 5-fold CV → Model selection → Deployment

    **Models:** Linear Regression · Random Forest · Gradient Boosting

    **Metrics:** R² · MAE · RMSE on a held-out 20% test set

    > *Trained on synthetic data. Predictions are illustrative — not financial advice.*
    """)

# ── Footer ────────────────────────────────────────────────────────────────────

st.markdown("""
<div style="text-align:center;color:#334155;font-size:0.82rem;margin-top:3rem;
